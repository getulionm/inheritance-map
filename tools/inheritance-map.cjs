...
if (!rootArg) {
  console.error("Usage: node tools/inheritance-map.cjs --root <folder> [--out <file.md>]");
  console.error("  Output default: <folder-basename>-inheritance-map.md in the current working directory.");
  process.exit(1);
}

const root = path.resolve(rootArg);
const folderBasename = path.basename(root);
const outPath = outArg ? path.resolve(outArg) : path.resolve(`${folderBasename}-inheritance-map.md`);
